...
module quickhull (S: euclidean_space) : convex_hull with space.point = S.point = {
  ...
  def expand_hull [num_segs] [num_points]
                  ...
                  (points: [num_points](i32, point)) : ([](point, point), [](i32, point)) =
    let dists =
      map (\(seg_ix, p) ->
    ...
    let extrema_ix =
      reduce_by_index (replicate num_segs (-1, zero_dist))
                      max
                      (-1, zero_dist)
                      (map (i64.i32 <-< (.0)) points)
                      (zip (iota num_points) dists)
    let segs' =
      tabulate num_segs
               (\i ->
    ...
    let eval_point (ix, (seg_ix, p)) =
      if extrema_ix[seg_ix].0 == ix
      then (-1, p)
      else let (a, b) = segs[seg_ix]
           let q = points[extrema_ix[seg_ix].0].1
           let daq = signed_dist_to_line a q p
           let dqb = signed_dist_to_line q b p
           in if dist_less zero_dist daq
              then (seg_ix * 2, p)
              else if dist_less zero_dist dqb
              then (seg_ix * 2 + 1, p)
              else (-1, p)
    let points' =
      filter ((>= 0) <-< (.0))
  ...
  def extract_empty_segments [num_segs] [num_points]
                             (hull: [](point))
                             (segs: [num_segs](point, point))
                             (points: [num_points](i32, point)) : ([](point), [](point, point), [](i32, point)) =
    let point_ixs = map (i64.i32 <-< (.0)) points
    let segs_inhabited =
      reduce_by_index (replicate num_segs 0i32) (+) 0 point_ixs (replicate num_points 1)
      |> map (> 0)
  ...
  def compute (ps: []point) =
    if length ps <= 3
    then (ps, [])
    else let leftmost = reduce pmin ps[0] ps
         let rightmost = reduce pmax ps[0] ps
         let (_, upper_points, lower_points) =
           partition2 (\p -> point_eq p leftmost || point_eq p rightmost)
...
def main [k] (ps: [k][2]f64) : []i32 =
  let ps' =
    map2 (\i p -> ({x = f32.f64 p[0], y = f32.f64 p[1]}, i32.i64 i))
         (indices ps)
         ps
