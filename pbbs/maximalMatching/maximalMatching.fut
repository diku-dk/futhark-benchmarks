...
def update [arraySize]
           (edges: [arraySize][2]i32)
           (edgeIds: [arraySize]i32)
           (smallestEdgeId: []i32)
           (markedVerts: *[]bool)
           (includedEdges: *[]bool) : (*[]bool, *[]bool) =
  -- The length of the flattened arrays
  let arraySizeFlat = arraySize * 2
...
def MM [nVerts] [nEdges]
       (edges: [][2]i32)
       (edgeIds_all: [nEdges]i64)
       (markedVerts: *[nVerts]bool)
       (smallestEdgeId: *[nVerts]i32)
       (includedEdges: *[nEdges]bool) =
  let edgeIds = map i32.i64 edgeIds_all
  let (_, _, _, _, includedEdges) =
    loop (edges, edgeIds, markedVerts, smallestEdgeId, includedEdges) while (length edges > 0) do
