...
def find (UFparents: []i32) (vert: i32) : i32 =
  ...
  then vert
  else -- Due to agressive flattening, this should never actually have to loop
       loop p = UFparents[vert]
       while !(is_root32 UFparents p) do
         UFparents[p]

def find2 (UFparents: []i32) (edge: (i32, i32)) : (i32, i32) =
...
def getLowestIndexes [arraySize] (edges: [arraySize](i32, i32)) (edgeIds: [arraySize]i64) (nVerts: i64) (nEdges: i64) =
...
def getMSFEdges (smallestEdgeId: []i64) (e: (i32, i32)) (i: i64) : ((i32, i32), i64) =
  if (smallestEdgeId[e.1] == i)
  then ((e.1, e.0), i)
  else if (smallestEdgeId[e.0] == i)
  then ((e.0, e.1), i)
  else ((-1, -1), -1)

-- Update the UF array and the array of included edges
def update [arraySize]
           (UFparents: *[]i32)
           (edges: [arraySize](i32, i32))
...
def MSF [nVerts] [nEdges]
  ...
  let (_, _, _, _, includedEdges) =
    loop (UFparents, edges, edgeIds, smallestEdgeId, includedEdges) while (length edges > 0) do
...
def main [nEdges] (edges_enc: [nEdges][2]i32) (weights: [nEdges]f64) =
  let nVerts = flatten edges_enc |> i32.maximum |> (+ 1) |> i64.i32
  let edges = map (\e -> (e[0], e[1])) edges_enc
