...
def triangle_hit ({orig = o, dir = d}: ray) ({a, b, c, index = _}: triangle) : f32 =
  ...
  in if det > -EPSILON && det < EPSILON
     then 0
     else -- Calculate distance from 'a' to origin
          let invDet = 1 / det
          ...
          in if u < 0.0 || u > 1.0
             then 0
             else let qvec = vec3.cross tvec e1
...
def triangle_aabb ({a, b, c, index = _}: triangle) : aabb =
  { min =
      { x = a.x `f32.min` b.x `f32.min` c.x
      , y = a.y `f32.min` b.y `f32.min` c.y
      , z = a.z `f32.min` b.z `f32.min` c.z
      }
  , max =
      { x = a.x `f32.max` b.x `f32.max` c.x
...
def main [npoints] [ntriangles] [nrays]
         ...
         (rays_enc: [nrays][2][3]f32) : [nrays]i32 =
  ...
  -- actually 1-based.
  let to_triangle t i = {a = points[t[0] - 1], b = points[t[1] - 1], c = points[t[2] - 1], index = i32.i64 i}
