...
def xorInds [num_bits] (n: i32) (dir_vs: [num_bits]i32) : i32 =
  let reldv_vals =
    map2 (\dv i -> if testBit (grayCode n, i32.i64 i) then dv else 0)
         dir_vs
         (indices dir_vs)
...
def brownianBridge [num_dates]
                   ...
                   (bb_inds: [3][num_dates]i32)
                   (bb_data: [3][num_dates]f32)
                   (gauss2d: [num_dates][num_und]f32) : [num_dates][num_und]f32 =
...
def blackScholes [num_dates] [num_und]
                 ...
                 (md_vols: [num_dates][num_und]f32)
                 (md_drifts: [num_dates][num_und]f32)
                 (md_starts: [num_und]f32) : [num_dates][num_und]f32 =
...
def payoff2 (md_disc: []f32, xss: [5][3]f32) : f32 =
  let (date, amount) =
    if 1.0 <= fminPayoff (xss[0])
    then (0, 1150.0)
    else if 1.0 <= fminPayoff (xss[1])
    then (1, 1300.0)
    else if 1.0 <= fminPayoff (xss[2])
    then (2, 1450.0)
    else if 1.0 <= fminPayoff (xss[3])
    then (3, 1600.0)
...
def genericPayoff (contract: i32) (md_disct: []f32) (md_detval: []f32) (xss: [][]f32) : f32 =
  if contract == 1
  then #[unsafe] payoff1 (md_disct, md_detval, xss :> [1][1]f32)
  else if contract == 2
  then #[unsafe] payoff2 (md_disct, xss :> [5][3]f32)
  else if contract == 3
  then #[unsafe] payoff3 (md_disct, xss :> [367][3]f32)
  else 0.0
...
def main [num_bits] [num_models] [num_und] [num_dates] [num_discts]
  ...
  let payoffs =
    map (\bb_row ->
           map3 (genericPayoff contract_number)
                md_discts
                md_detvals
