...
fun initGrid(s0: f32, alpha: f32, nu: f32, t: f32, numX: int, numY: int, numT: int)
  ...
  let myTimeline = map (fn i: f32  => t * f32 i / (f32 numT - 1.0)) (iota numT)
  let (stdX, stdY) = (20.0 * alpha * s0 * sqrt32(t),
                      10.0 * nu         * sqrt32(t))
  let (dx, dy) = (stdX / f32 numX, stdY / f32 numY)
  let (myXindex, myYindex) = (int(s0 / dx), numY / 2)
  let myX = map (fn i: f32 => f32(i) * dx - f32 myXindex * dx + s0) (iota numX)
  let myY = map (fn i: f32 => f32(i) * dy - f32 myYindex * dy + logAlpha) (iota numY)
