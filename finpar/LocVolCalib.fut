...
fun initGrid(s0: f32, alpha: f32, nu: f32, t: f32, numX: int, numY: int, numT: int): (int,int,[numX]f32,[numY]f32,[numT]f32) =
    ...
    let myTimeline = map (fn (i: int): f32  => t * f32(i) / (f32(numT) - 1.0)) (iota(numT))
    let (stdX, stdY) = (20.0 * alpha * s0 * sqrt32(t),
                        10.0 * nu         * sqrt32(t))
    let (dx, dy) = (stdX / f32(numX), stdY / f32(numY))
    let (myXindex, myYindex) = (int(s0 / dx), numY / 2)
    let myX = map (fn (i: int): f32  => f32(i) * dx - f32(myXindex) * dx + s0) (iota(numX))
    let myY = map (fn (i: int): f32  => f32(i) * dy - f32(myYindex) * dy + logAlpha) (iota(numY))
...
fun tridagPar(a:  [n]f32, b: *[]f32, c: []f32, y: *[]f32 ): *[]f32 =
    ...
    let y    = map  (fn (i: int): f32  => y[n-i-1]) (iota(n))
    in y
...
fun implicitMethod(myD:  [][]f32,  myDD: [][]f32,
  ...
  map (fn (tup:  ([]f32,[]f32,*[]f32) ): *[]f32   =>
         ...
         let abc = map (fn (tup: (f32,f32,[]f32,[]f32)): (f32,f32,f32)  =>
                          let (mu, var, d, dd) = tup
                          in ( 0.0   - 0.5*(mu*d[0] + 0.5*var*dd[0])
...
fun rollback
    ...
    let u = map (fn (tup: ([]f32,[]f32)): []f32  =>
                    let (u_row, res_row) = tup
                    in map  (fn (tup: (f32,f32)): f32  =>
                               let (u_el,res_el) = tup
    ...
    let y = map (fn (uv_row: ([]f32,[]f32)): []f32  =>
                   let (u_row, v_row) = uv_row
                   in map (fn (uv: (f32,f32)): f32  =>
                             let (u_el,v_el) = uv
...
fun main (outer_loop_count: int, numX: int, numY: int, numT: int,
    ...
    let strikes = map (fn (i: int): f32  => 0.001*f32(i)) (iota(outer_loop_count))
    let res = map (fn (x: f32): f32  => value(numX, numY, numT, s0, x, t, alpha, nu, beta)) strikes
    in res
