...
def initGrid (s0: f32) (alpha: f32) (nu: f32) (t: f32) (numX: i64) (numY: i64) (numT: i64) : (i32, i32, [numX]f32, [numY]f32, [numT]f32) =
  let logAlpha = f32.log alpha
  let myTimeline = map (\i -> t * f32.i64 i / (f32.i64 numT - 1.0)) (iota numT)
  let (stdX, stdY) =
    ( 20.0 * alpha * s0 * f32.sqrt (t)
    , 10.0 * nu * f32.sqrt (t)
    )
...
def tridagPar [n] (a: [n]f32, b: [n]f32, c: [n]f32, y: [n]f32) : *[n]f32 =
  ...
  let cfuns =
    scan (\(a0, a1) (b0, b1) -> (b0 + b1 * a0, a1 * b1))
         (0.0, 1.0)
         lfuns
...
def rollback [numX] [numY]
             ( tnow: f32
             , tnext: f32
             , myResult: [numY][numX]f32
             , myMuX: [numY][numX]f32
             , myDx: [numX][3]f32
             , myDxx: [numX][3]f32
             , myVarX: [numY][numX]f32
             , myMuY: [numX][numY]f32
             , myDy: [numY][3]f32
             , myDyy: [numY][3]f32
             , myVarY: [numX][numY]f32
             ) : [numY][numX]f32 =
  ...
  -- implicitY
  let y =
    map2 (map2 (\u_el v_el -> dtInv * u_el - 0.5 * v_el))
         (transpose u)
         v
...
def value (numX: i64, numY: i64, numT: i64, s0: f32, strike: f32, t: f32, alpha: f32, nu: f32, beta: f32) : f32 =
  ...
  let myResult =
    loop (myResult) for (tnow, tnext) in myTimeline_neighbours do
