...
def thefunction (x: f64, omegan: f64, select: i32) : f64 =
  if select == 0
  then (x + 1.0) ** x
  else if select == 1
  then (x + 1.0) ** x * f64.cos (omegan * x)
  else (x + 1.0) ** x * f64.sin (omegan * x)

def TrapezoidIntegrate (x0: f64, x1: f64, nsteps: i32, omegan: f64, select: i32) : f64 =
  let x = x0
  let dx = (x1 - x0) / f64.i32 nsteps
...
def main (array_rows: i64) : ([array_rows]f64, [array_rows]f64) =
  let omega = 3.1415926535897932
  let first =
    [ ( TrapezoidIntegrate (0.0, 2.0, 1000, 0.0, 0) / 2.0
      , 0.0
      )
    ]
  -- never set in reference implementation

  let rest =
    map (\i ->
           ( TrapezoidIntegrate (0.0, 2.0, 1000, omega * f64.i64 i, 1)
           , TrapezoidIntegrate (0.0, 2.0, 1000, omega * f64.i64 i, 2)
           ))
