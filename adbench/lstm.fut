...
def lstmObjective [stlen] [lenSeq] [d]
  ...
  let (_, total) =
    loop (oldState, oldTotal) = (state, 0f64)
    for i < lenSeq - 1 do
      let (y_pred, newState) = lstmPredict mainParams extraParams oldState sequence[i]
