...
module vector_1 : vector = {
  ...
  def map f a = copy (f a)
  def map2 f a b = copy (f a b)
  def reduce f ne a = copy (f ne a)
  def zip a b = copy (a, b)
  def vzip = copy
  def vunzip = copy
  def iota = 0i64
  def replicate a = copy a
  def get _ a = a
  def set _ x _ = copy x
  def length = 1i64
  def foldl f b x = f b x
  def foldr f b x = f x b
  def to_array a = stdreplicate length a
  def from_array as = copy as[0]
}
...
module cat_vector (X: vector) (Y: vector) : vector = {
  ...
  def reduce f ne (xs, ys) = copy (X.reduce f ne xs `f` Y.reduce f ne ys)
  ...
  def set 'a i v (xs: *X.vector a, ys: *Y.vector a) =
    if i < X.length
    then (X.set i v xs, ys)
  ...
  def from_array 'a as : *vector a =
    let xs = X.from_array (take X.length as)
