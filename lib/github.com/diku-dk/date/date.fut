...
-- intent is to make the type of dates abstract.
...
module type date = {
  ...
  -- | Convert a date to a triple of (year,month,day).  Months and
  -- days are 1-indexed (technically, so are years, but nobody is
  -- surprised by that).  The time is assumed to be 12:00 that day.
  val triple_of_date : date -> (i32, i32, i32)

  -- | The inverse of `triple_of_date`@term.  The result is undefined
  -- if the triple does not describe a valid date.
