...
module mk_ordered_linalg (T: ordered_field) : ordered_linalg with t = T.t = {
  ...
  def vhouse [n] (x: [n]t) : ([n]t, t) =
    ...
    let x = if T.isnan normx || T.isinf normx then x else vecscale normx x
    --OK?
    let s = dotprod x[1:] x[1:]
    let v = [T.i64 1] ++ x[1:] :> [n]t
    in if T.(s == i64 0)
       then (v, T.i64 0)
       else let mu = T.(sqrt (x[0] ** (i64 2) + s))
  ...
  def eig [n] (A: [n][n]t) : ([n][n]t, [n][n]t) =
    ...
    let (H, U) =
      loop (H, U) = (copy H, copy U)
      for j in 0..<n - 1 do
        let m = n - j
        let I = eye m
        in loop (H, U) = (H, U)
           while (T.abs H[m - 1, m - 2]) T.> epsilon do
             let sigma = H[m - 1, m - 1]
             let HmmI = matscale sigma I
             let H = H with [:m, :m] = matsub H[:m, :m] HmmI
             --Rayleigh quotient shift
             let (cs, ss) = (veczeros m, veczeros m)
             let (H, cs, ss) =
               loop (H, cs, ss) = (H, cs, ss)
               for i in 0..<m - 1 do
                 let (c, s) = givens H[i, i] H[i + 1, i]
                 --these must be calculated iteratively, and ONLY IN THIS LOOP!
                 let cs = cs with [i] = c
                 let ss = ss with [i] = s
             ...
             let (H, U) =
               ...
               for i in 0..<m - 1 do
                 ...
                 in (H, U)
             let H = H with [:m, :m] = matadd H[:m, :m] HmmI
             --Rayleigh quotient "un"shift
             in (H, U)
    let D = matdiag H
    ...
    let V =
      loop V = V
      for i in 2...n do
        --The columns of V are now the eigenvectors of the Hessenberg form H of A
        let im = i - 1
        let subH = H[:i - 1, :i - 1] :> [im][im]t
        let subV = V[:i - 1, :i - 1] :> [im][im]t
        let Hminus = H[:i - 1, i - 1] :> [im]t
        let lam = D[i - 1, i - 1]
        let X = matsub (matscale lam subV) (matmul subH subV)
        --this eigenvector is a linear combo of the previous
        let a =
          loop a = matones i 1
          for j in i - 2..>-1 do
            --now do backsubstitution to solve
            let jp = i - 1 - (j + 1)
