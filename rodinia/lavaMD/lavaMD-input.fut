...
-----------------------
-----------------------
def main (boxes1d: i64) (par_per_box: i64) (num_neighbors: i64) : ( f32
                                                                  , []i32
                                                                  , []i32
                                                                  , []i32
                                                                  , []i32
  ...
  let boxes =
    tabulate number_boxes
             (\nh ->
                ...
                let (box_nngh_cur_nn) =
                  loop (box_nngh, cur_nn) for nn < num_neighbors do
                    let n = (nn % 3) - 1
                    let nr = nn / 3
                    let m = (nr % 3) - 1
                    let l = (nr / 3) - 1
    ...
    map (\i ->
           map (\j ->
                  ...
                  in ( f32.i32 (s5 % 10 + 1) / 10.0
                     , ( f32.i32 (s1 % 10 + 1) / 10.0
                       , f32.i32 (s2 % 10 + 1) / 10.0
                       , f32.i32 (s3 % 10 + 1) / 10.0
                       , f32.i32 (s4 % 10 + 1) / 10.0
                       )
                     ))
               (iota (par_per_box)))
        (iota (number_boxes))
