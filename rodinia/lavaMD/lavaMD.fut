...
def main [number_boxes] [par_per_box] [num_neighbors]
  ...
  let a2 = 2.0 * alpha * alpha
  in unzip4
     <| map unzip4 (map2 (\box_num_nghbs' (l: i64) : [par_per_box](f32, f32, f32, f32) ->
                            let rA = rv[l]
                            in map (\(rA_el: (f32, f32, f32, f32)) : (f32, f32, f32, f32) ->
                                      --(i32 i) ->
                                      let (rai_v, rai_x, rai_y, rai_z) = rA_el
                                      -- rA[i]
