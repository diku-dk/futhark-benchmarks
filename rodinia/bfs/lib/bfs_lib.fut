...
-- element.
let i32_excl_scan_from_incl_scan (scanned: [k]i32) (ne: i32): [k]i32 =
  map (\(i: i32): i32  -> if i == 0 then ne else unsafe scanned[i - 1]) (iota(k))

-- Like scanSegm((+), 0, array, mask).
let i32_plus_scan_segm(array: [k]i32, mask: [k]bool): [k]i32 =
...
let get_updating_indices_alt0(updating_graph_mask: [n]bool): (*[]i32, i32) =
...
-- and large datasets.
let get_updating_indices_alt1(updating_graph_mask: [n]bool): (*[n]i32, i32) =
