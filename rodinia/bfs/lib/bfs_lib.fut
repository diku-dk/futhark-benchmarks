...
-- function).
fun *[]i32 i32_filter([k]bool bs, [k]i32 ns) =
...
-- element.
fun [k]i32 i32_excl_scan_from_incl_scan([k]i32 scanned, i32 ne) =
  map(fn i32 (i32 i) => if i == 0 then ne else unsafe scanned[i - 1], iota(k))

-- Like scanSegm(+, 0, array, mask).
fun [k]i32 i32_plus_scan_segm([k]i32 array, [k]bool mask) =
...
-- and large datasets.
fun (*[n]i32, i32) get_updating_indices_alt1([n]bool updating_graph_mask) =
