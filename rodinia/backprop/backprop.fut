...
def bpnn_hidden_error [no] [nh] (delta_o: [no]f32, who: [nh][no]f32, hidden: [nh]f32) : (f32, [nh]f32) =
  let (errs, delta_h) =
    unzip (map (\(hidden_el: f32, who_row: []f32) : (f32, f32) ->
...
def sobolIndR [num_bits] (dirVct: [num_bits]i32) (n: i32) : f32 =
  ...
  let res = 0
  let res =
    loop (res) for (i, v) in (zip (iota num_bits) dirVct) do
      let t = i32.i64 (1 << i)
      in if (n_gray & t) == t
         then res ^ v
         else res
...
def bpnn_create (n_in: i64) (n_hid: i64) (n_out: i64) (offset: i32) (dirVct: []i32) : ( [n_in]f32
                                                                                      , [n_out]f32
