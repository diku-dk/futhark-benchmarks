...
fun centroids_of(k: int, points: [n](f32,f32), membership: [n]int): *[k](f32,f32) =
  let (cluster_counts, cluster_points) =
    unzip(map (\(cluster: int): [n](int,(f32,f32))  ->
                map (\(point_cluster: int) (point: (f32,f32)): (int, (f32,f32))  ->
                          if cluster == point_cluster
                          then (1, point)
                          else (0, (0.0f32, 0.0f32))) membership points) (
                iota(k)))
  let cluster_sizes = map (\(counts: [n]int): int  ->
                            reduce (+) 0 counts) (
                          cluster_counts)
  let cluster_centres = map (\(count: int) (my_points: [n](f32,f32)): (f32,f32)  ->
                                  let (x,y) =
...
fun main(threshold: int,
  ...
  let cluster_centres = map (\(i: int): (f32,f32)  ->
                              unsafe points[i]) (
                            iota(k))
