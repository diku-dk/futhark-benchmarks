...
def ecc [equs] [pars]
  ...
  let cmem = 1.3810e-10f32
  -- [F] membrane capacitance
  let qpow = (temp - 310.0f32) / 10.0f32
  -- Cell geometry
  let cellLength = 100.0f32
  -- cell length []um
  let cellRadius = 10.25f32
  -- cell radius []um
  let vcell = pi * pow (cellRadius, 2.0f32) * cellLength * 1.0e-15f32
  -- [L]
  let vmyo = 0.65f32 * vcell
  let vsr = 0.035f32 * vcell
  let vsl = 0.02f32 * vcell
  let vjunc = 0.0539f32 * 0.01f32 * vcell
  let j_ca_juncsl = 1.0f32 / 1.2134e12f32
  -- [L/msec]
  let j_ca_slmyo = 1.0f32 / 2.68510e11f32
  -- [L/msec]
  let j_na_juncsl = 1.0f32 / (1.6382e12f32 / 3.0f32 * 100.0f32)
  -- [L/msec]
  let j_na_slmyo = 1.0f32 / (1.8308e10f32 / 3.0f32 * 100.0f32)
  -- [L/msec]
  ...
  let mgi = 1.0f32
  -- Nernst Potentials
  let ena_junc = (1.0f32 / foRT) * f32.log (nao / initvalu_32)
  -- []mV
  let ena_sl = (1.0f32 / foRT) * f32.log (nao / initvalu_33)
  -- []mV
  let ek = (1.0f32 / foRT) * f32.log (ko / initvalu_35)
  -- []mV
  let eca_junc = (1.0f32 / foRT / 2.0f32) * f32.log (cao / initvalu_36)
  -- []mV
  let eca_sl = (1.0f32 / foRT / 2.0f32) * f32.log (cao / initvalu_37)
  -- []mV
  let ecl = (1.0f32 / foRT) * f32.log (cli / clo)
  -- []mV

  -- Na transport parameters
  let gna = 16.0f32
  -- [#mS/uF]
  let gnaB = 0.297e-3f32
  -- [#mS/uF]
  let iBarNaK = 1.90719f32
  -- [#uA/uF]
  let kmNaip = 11.0f32
  -- []mM
  let kmko = 1.5f32
  ...
  let gClCa = 0.109625f32
  -- [#mS/uF]
  let gClB = 9.0e-3f32
  -- [#mS/uF]
  let kdClCa = 100.0e-3f32
  -- []mM
  ...
  let nu = 0.35f32
  -- []none
  let kdact = 0.256e-3f32
  -- []mM
  let q10NCX = 1.57f32
  -- []none
  let ibarSLCaP = 0.0673f32
  -- [#uA/uF]
  let kmPCa = 0.5e-3f32
  -- []mM
  let gCaB = 2.513e-4f32
  -- [#uA/uF]
  let q10SLCaP = 2.35f32
  -- []none

  -- SR flux parameters
  let q10SRCaP = 2.6f32
  -- []none
  let vmax_SRCaP = 2.86e-4f32
  -- [#mM/msec] (mmol/L cytosol/msec)
  let kmf = 0.246e-3f32
  -- []mM
  let kmr = 1.7f32
  -- []mML cytosol
  let hillSRCaP = 1.787f32
  -- []mM
  let ks = 25.0f32
  -- [1/ms]
  let koCa = 10.0f32
  ...
  let bmax_Naj = 7.561f32
  -- []mM
  let bmax_Nasl = 1.65f32
  -- []mM
  let koff_na = 1.0e-3f32
  -- [1/ms]
  let kon_na = 0.1e-3f32
  -- [1/mM/ms]
  let bmax_Tnclow = 70e-3f32
  -- []mM, TnC low affinity
  let koff_tncl = 19.6e-3f32
  -- [1/ms]
  let kon_tncl = 32.7f32
  -- [1/mM/ms]
  let bmax_TnChigh = 140.0e-3f32
  -- []mM, TnC high affinity
  let koff_tnchca = 0.032e-3f32
  -- [1/ms]
  let kon_tnchca = 2.37f32
  -- [1/mM/ms]
  let koff_tnchmg = 3.33e-3f32
  -- [1/ms]
  let kon_tnchmg = 3.0e-3f32
  -- [1/mM/ms]
  let bmax_myosin = 140.0e-3f32
  -- []mM, Myosin buffering
  let koff_myoca = 0.46e-3f32
  -- [1/ms]
  let kon_myoca = 13.8f32
  -- [1/mM/ms]
  let koff_myomg = 0.057e-3f32
  -- [1/ms]
  let kon_myomg = 0.0157f32
  -- [1/mM/ms]
  let bmax_SR = 19.0f32 * 0.9e-3f32
  -- []mM
  let koff_sr = 60.0e-3f32
  -- [1/ms]
  let kon_sr = 100.0f32
  -- [1/mM/ms]
  let bmax_SLlowsl = 37.38e-3f32 * vmyo / vsl
  -- []mM, SL buffering
  let bmax_SLlowj = 4.62e-3f32 * vmyo / vjunc * 0.1f32
  -- []mM
  let koff_sll = 1300.0e-3f32
  -- [1/ms]
  let kon_sll = 100.0f32
  -- [1/mM/ms]
  let bmax_SLhighsl = 13.35e-3f32 * vmyo / vsl
  -- []mM
  let bmax_SLhighj = 1.65e-3f32 * vmyo / vjunc * 0.1f32
  -- []mM
  let koff_slh = 30e-3f32
  -- [1/ms]
  let kon_slh = 100.0f32
  -- [1/mM/ms]
  let bmax_Csqn = 2.7f32
  -- 140e-3*vmyo/Vsr; []mM
  let koff_csqn = 65.0f32
  -- [1/ms]
  let kon_csqn = 100.0f32
  -- [1/mM/ms]
  ...
  let (ah, aj, bh, bj) =
    ...
    then ( 0.0f32
         ...
         , (-127140.0f32 * f32.exp (0.2444f32 * initvalu_39) - 3.474e-5f32 * f32.exp (-0.04391f32 * initvalu_39))
           * (initvalu_39 + 37.78f32)
           / (1.0f32 + f32.exp (0.311f32 * (initvalu_39 + 79.23f32)))
  ...
  let tauxr =
    1.0f32
    / (0.00138f32 * (initvalu_39 + 7.0f32) / (1.0f32 - f32.exp (-0.123f32 * (initvalu_39 + 7.0f32)))
  ...
  let tauxs =
    1.0f32
    / (7.19e-5f32 * (initvalu_39 + 30.0f32) / (1.0f32 - f32.exp (-0.148f32 * (initvalu_39 + 30.0f32)))
  ...
  let i_tos = dToSlow * initvalu_8 * (initvalu_9 + 0.5f32 * initvalu_40) * (initvalu_39 - ek)
  -- [#uA/uF]
  ...
  let bki =
    (0.49124f32 * f32.exp (0.08032f32 * (initvalu_39 + 5.476f32 - ek)) + f32.exp (0.06175f32 * (initvalu_39 - ek - 594.31f32)))
  ...
  let finavalu[offset_6] = 1.7f32 * initvalu_36 * (1.0f32 - initvalu_6) - 11.9e-3f32 * initvalu_6
  -- fCa_junc
  let finavalu[offset_7] = 1.7f32 * initvalu_37 * (1.0f32 - initvalu_7) - 11.9e-3f32 * initvalu_7
  -- fCa_sl

  --
  let ibarca_j =
    pCa * 4.0f32 * (initvalu_39 * frdy * foRT) * (0.341f32 * initvalu_36 * f32.exp (2.0f32 * initvalu_39 * foRT) - 0.341f32 * cao)
    / (f32.exp (2.0f32 * initvalu_39 * foRT) - 1.0f32)
  let ibarca_sl =
    pCa * 4.0f32 * (initvalu_39 * frdy * foRT) * (0.341f32 * initvalu_37 * f32.exp (2.0f32 * initvalu_39 * foRT) - 0.341f32 * cao)
    / (f32.exp (2.0f32 * initvalu_39 * foRT) - 1.0f32)
  let ibark =
    pK * (initvalu_39 * frdy * foRT) * (0.75f32 * initvalu_35 * f32.exp (initvalu_39 * foRT) - 0.75f32 * ko)
    / (f32.exp (initvalu_39 * foRT) - 1.0f32)
  let ibarna_j =
    pNa * (initvalu_39 * frdy * foRT) * (0.75f32 * initvalu_32 * f32.exp (initvalu_39 * foRT) - 0.75f32 * nao)
    / (f32.exp (initvalu_39 * foRT) - 1.0f32)
  let ibarna_sl =
    pNa * (initvalu_39 * frdy * foRT) * (0.75f32 * initvalu_33 * f32.exp (initvalu_39 * foRT) - 0.75f32 * nao)
    / (f32.exp (initvalu_39 * foRT) - 1.0f32)
  ...
  let s3_junc =
    (kmCai * pow (nao, 3.0f32) * (1.0f32 + pow ((initvalu_32 / kmNai), 3.0f32)) + pow (kmnao, 3.0f32) * initvalu_36
     + pow (kmNai, 3.0f32) * cao * (1.0f32 + initvalu_36 / kmCai)
     + kmcao * pow (initvalu_32, 3.0f32)
     + pow (initvalu_32, 3.0f32) * cao
     + pow (nao, 3.0f32) * initvalu_36)
    * (1.0f32 + ksat * f32.exp ((nu - 1.0f32) * initvalu_39 * foRT))
  let s2_sl = f32.exp ((nu - 1.0f32) * initvalu_39 * foRT) * pow (nao, 3.0f32) * initvalu_37
  let s3_sl =
    (kmCai * pow (nao, 3.0f32) * (1.0f32 + pow ((initvalu_33 / kmNai), 3.0f32))
     + pow (kmnao, 3.0f32) * initvalu_37
     + pow (kmNai, 3.0f32) * cao * (1.0f32 + initvalu_37 / kmCai)
     + kmcao * pow (initvalu_33, 3.0f32)
     + pow (initvalu_33, 3.0f32) * cao
     + pow (nao, 3.0f32) * initvalu_37)
    * (1.0f32 + ksat * f32.exp ((nu - 1.0f32) * initvalu_39 * foRT))
  ...
  let finavalu[offset_14] =
    (kim * ri - kiSRCa * initvalu_36 * initvalu_14)
    - (koSRCa * pow (initvalu_36, 2.0f32) * initvalu_14 - kom * initvalu_15)
  -- R
  let finavalu[offset_15] =
    (koSRCa * pow (initvalu_36, 2.0f32) * initvalu_14 - kom * initvalu_15)
    - (kiSRCa * initvalu_36 * initvalu_15 - kim * initvalu_16)
  -- O
  let finavalu[offset_16] =
    (kiSRCa * initvalu_36 * initvalu_15 - kim * initvalu_16)
    - (kom * initvalu_16 - koSRCa * pow (initvalu_36, 2.0f32) * ri)
  -- i
  let j_SRCarel = ks * initvalu_15 * (initvalu_31 - initvalu_36)
  -- [#mM/ms]
  let j_serca =
    pow (q10SRCaP, qpow) * vmax_SRCaP * (pow ((initvalu_38 / kmf), hillSRCaP) - pow ((initvalu_31 / kmr), hillSRCaP))
    / (1.0f32 + pow ((initvalu_38 / kmf), hillSRCaP) + pow ((initvalu_31 / kmr), hillSRCaP))
  let j_SRleak = 5.348e-6f32 * (initvalu_31 - initvalu_36)
  -- [#mM/ms]

  -- Sodium and Calcium Buffering
  let finavalu[offset_17] = kon_na * initvalu_32 * (bmax_Naj - initvalu_17) - koff_na * initvalu_17
  -- NaBj  [#mM/ms]
  let finavalu[offset_18] = kon_na * initvalu_33 * (bmax_Nasl - initvalu_18) - koff_na * initvalu_18
  -- NaBsl [#mM/ms]
  -- Cytosolic Ca Buffers
  let finavalu[offset_19] = kon_tncl * initvalu_38 * (bmax_Tnclow - initvalu_19) - koff_tncl * initvalu_19
  -- TnCL [#mM/ms]
  let finavalu[offset_20] = kon_tnchca * initvalu_38 * (bmax_TnChigh - initvalu_20 - initvalu_21) - koff_tnchca * initvalu_20
  -- TnCHc [#mM/ms]
  let finavalu[offset_21] = kon_tnchmg * mgi * (bmax_TnChigh - initvalu_20 - initvalu_21) - koff_tnchmg * initvalu_21
  -- TnCHm [#mM/ms]
  let finavalu[offset_22] = 0.0f32
  -- CaM [#mM/ms]
  let finavalu[offset_23] = kon_myoca * initvalu_38 * (bmax_myosin - initvalu_23 - initvalu_24) - koff_myoca * initvalu_23
  -- Myosin_ca [#mM/ms]
  let finavalu[offset_24] = kon_myomg * mgi * (bmax_myosin - initvalu_23 - initvalu_24) - koff_myomg * initvalu_24
  -- Myosin_mg [#mM/ms]
  let finavalu[offset_25] = kon_sr * initvalu_38 * (bmax_SR - initvalu_25) - koff_sr * initvalu_25
  -- SRB [#mM/ms]
  let j_CaB_cytosol =
    finavalu[offset_19] + finavalu[offset_20] + finavalu[offset_21]
    + finavalu[offset_22]
    + finavalu[offset_23]
    + finavalu[offset_24]
    + finavalu[offset_25]
  -- Junctional and SL Ca Buffers
  let finavalu[offset_26] = kon_sll * initvalu_36 * (bmax_SLlowj - initvalu_26) - koff_sll * initvalu_26
  -- SLLj  [#mM/ms]
  let finavalu[offset_27] = kon_sll * initvalu_37 * (bmax_SLlowsl - initvalu_27) - koff_sll * initvalu_27
  -- SLLsl [#mM/ms]
  let finavalu[offset_28] = kon_slh * initvalu_36 * (bmax_SLhighj - initvalu_28) - koff_slh * initvalu_28
  -- SLHj  [#mM/ms]
  let finavalu[offset_29] = kon_slh * initvalu_37 * (bmax_SLhighsl - initvalu_29) - koff_slh * initvalu_29
  -- SLHsl [#mM/ms]
  let j_CaB_junction = finavalu[offset_26] + finavalu[offset_28]
  let j_CaB_sl = finavalu[offset_27] + finavalu[offset_29]
  -- SR Ca Concentrations
  let finavalu[offset_30] = kon_csqn * initvalu_31 * (bmax_Csqn - initvalu_30) - koff_csqn * initvalu_30
  -- Csqn [#mM/ms]
  let oneovervsr = 1.0f32 / vsr
  let finavalu[offset_31] = j_serca * vmyo * oneovervsr - (j_SRleak * vmyo * oneovervsr + j_SRCarel) - finavalu[offset_30]
  -- Ca_sr [#mM/ms]

  -- Sodium Concentrations
  let i_Na_tot_junc = i_Na_junc + i_nabk_junc + 3.0f32 * i_ncx_junc + 3.0f32 * i_nak_junc + i_CaNa_junc
  -- [#uA/uF]
  let i_Na_tot_sl = i_Na_sl + i_nabk_sl + 3.0f32 * i_ncx_sl + 3.0f32 * i_nak_sl + i_CaNa_sl
  -- [#uA/uF]
  ...
  let finavalu[offset_33] =
    -i_Na_tot_sl * cmem * oneovervsl / frdy + j_na_juncsl * oneovervsl * (initvalu_32 - initvalu_33)
    + j_na_slmyo * oneovervsl * (initvalu_34 - initvalu_33)
    - finavalu[offset_18]
  let finavalu[offset_34] = j_na_slmyo / vmyo * (initvalu_33 - initvalu_34)
  -- [#mM/msec]

  -- Potassium Concentration
  let i_K_tot = i_to + i_kr + i_ks + i_ki - 2.0f32 * i_nak + i_CaK + i_kp
  -- [#uA/uF]
  let finavalu[offset_35] = 0.0f32
  -- [#mM/msec]

  -- Calcium Concentrations
  let i_Ca_tot_junc = i_Ca_junc + i_cabk_junc + i_pca_junc - 2.0f32 * i_ncx_junc
  -- [#uA/uF]
  let i_Ca_tot_sl = i_Ca_sl + i_cabk_sl + i_pca_sl - 2.0f32 * i_ncx_sl
  -- [#uA/uF]
  let finavalu[offset_36] =
    -i_Ca_tot_junc * cmem / (vjunc * 2.0f32 * frdy)
    + j_ca_juncsl / vjunc * (initvalu_37 - initvalu_36)
    - j_CaB_junction
    + (j_SRCarel) * vsr / vjunc
    + j_SRleak * vmyo / vjunc
  -- Ca_j
  let finavalu[offset_37] =
    -i_Ca_tot_sl * cmem / (vsl * 2.0f32 * frdy)
  ...
  let i_app =
    if (fmod (timeinst, parameter_1) <= 5.0f32)
    then 9.5f32
    else 0.0f32
  -- Membrane Potential
  let i_Na_tot = i_Na_tot_junc + i_Na_tot_sl
  -- [#uA/uF]
  let i_Cl_tot = i_ClCa + i_Clbk
  -- [#uA/uF]
...
def cam [equs] [pars]
  ...
  let k20B = k20 / 100.0f32
  -- [#s^-1] thermo constraint on loop 1
  let k02B = k02
  -- [#uM^-2 s^-1]
  let k42B = k42
  -- [#s^-1] thermo constraint on loop 2
  let k24B = k24
  -- [#uM^-2 s^-1]

  -- Wi Wa Wt Wp
  let kbi = 2.2f32
  -- [#s^-1] (ca4caM dissocation from Wb)
  let kib = kbi / 33.5e-3f32
  -- [#uM^-1 s^-1]
  let kpp1 = 1.72f32
  -- [#s^-1] (pp1-dep dephosphorylation rates)
  let kmpp1 = 11.5f32
  -- []uM
  let kib2 = kib
  ...
  let kta = kbi / 1000.0f32
  -- [#s^-1] (ca4caM dissociation from Wt)
  let kat = kib
  -- [#uM^-1 s^-1] (ca4caM reassociation with Wa)
  let kt42 = k42 * 33.5e-6f32 / 5.0f32
  let kt24 = k24
  let kat2 = kib
  let kt2a = kib * 5.0f32
  -- caN parameters
  let kcancaoff = 1.0f32
  -- [#s^-1]
  let kcancaon = kcancaoff / 0.5f32
  -- [#uM^-1 s^-1]
  let kcancaM4on = 46.0f32
  -- [#uM^-1 s^-1]
  let kcancaM4off = 0.0013f32
  -- [#s^-1]
  ...
  -- caMkii equations
  let dpb2 = 1e-3f32 * (rcnCkib2 - rcnCkb2b + rcnCkt2b2)
  -- pb2
  let dpb = 1e-3f32 * (rcnCkib + rcnCkb2b - rcnCkbt)
  -- pb
  let dpt = 1e-3f32 * (rcnCkbt - rcnCkta - rcnCktt2)
  -- pt
  let dpt2 = 1e-3f32 * (rcnCktt2 - rcnCkt2a - rcnCkt2b2)
  -- pt2
  let dpa = 1e-3f32 * (rcnCkta + rcnCkt2a - rcnCkai)
  -- pa

  -- caN equations
  let dca4caN = 1e-3f32 * (rcnca4caN - rcn0caN - rcn2caN - rcn4caN)
  -- ca4caN
  let dcaMca4caN = 1e-3f32 * (rcn0caN - rcn02caN)
  -- caMca4caN
  let dca2caMca4caN = 1e-3f32 * (rcn2caN + rcn02caN - rcn24caN)
  -- ca2caMca4caN
  let dca4caMca4caN = 1e-3f32 * (rcn4caN + rcn24caN)
  -- ca4caMca4caN
  ...
  let jca =
    1e-3f32
    * (2.0f32 * caMKiitot * (rcnCktt2 - rcnCkb2b)
       - 2.0f32 * (rcn02 + rcn24 + rcn02B + rcn24B + rcnca4caN + rcn02caN + rcn24caN))
  -- [#uM/msec]
  in (jca, finavalu)
...
def fin [equs] [pars]
        (initvalu: [equs]f32)
        (initvalu_offset_ecc: i32)
        (initvalu_offset_Dyad: i32)
        (initvalu_offset_SL: i32)
        (initvalu_offset_Cyt: i32)
        (parameter: [pars]f32)
        (finavalu: *[equs]f32)
        (jcaDyad: f32)
        (jcaSL: f32)
  ...
  let caMtotDyad =
    initvalu[initvalu_offset_Dyad + 0]
    ...
    + caMKiitotDyad
      * (initvalu[initvalu_offset_Dyad + 6]
  ...
  let bdyad = btotDyad - caMtotDyad
  -- [#uM dyad]
  let j_cam_dyadSL =
    1e-3f32
    * (k0Boff * initvalu[initvalu_offset_Dyad + 0]
       - k0Bon * bdyad * initvalu[initvalu_offset_SL + 0])
  -- [#uM/msec dyad]
  let j_ca2cam_dyadSL =
    1e-3f32
    * (k2Boff * initvalu[initvalu_offset_Dyad + 1]
       - k2Bon * bdyad * initvalu[initvalu_offset_SL + 1])
  -- [#uM/msec dyad]
  let j_ca4cam_dyadSL =
    1e-3f32
    * (k2Boff * initvalu[initvalu_offset_Dyad + 2]
       - k4Bon * bdyad * initvalu[initvalu_offset_SL + 2])
  -- [#uM/msec dyad]

  let j_cam_SLmyo = kSLmyo * (initvalu[initvalu_offset_SL + 0] - initvalu[initvalu_offset_Cyt + 0])
  -- [#umol/msec]
  let j_ca2cam_SLmyo = kSLmyo * (initvalu[initvalu_offset_SL + 1] - initvalu[initvalu_offset_Cyt + 1])
  -- [#umol/msec]
  let j_ca4cam_SLmyo = kSLmyo * (initvalu[initvalu_offset_SL + 2] - initvalu[initvalu_offset_Cyt + 2])
  -- [#umol/msec]
  ...
  let (jcaDyad, jcaSL, jcaCyt, finavalu) =
    loop ((jcaDyad, jcaSL, jcaCyt, finavalu)) for ii < 3i32 do
      ...
      let (jcaDyad, jcaSL, jcaCyt) =
        if (ii == 0)
        then (res_val, jcaSL, jcaCyt)
        else if (ii == 1)
        then (jcaDyad, res_val, jcaCyt)
    ...
    loop (finavalu_temp) for ii < 13 do
      let initvalu_temp =
        map (\i ->
               ...
               then let x = initvalu[i]
                    let y = finavalu_temp[0, i]
                    in x + h2_7 * y
               else if ii == 2
               then let x = initvalu[i]
                    let y1 = finavalu_temp[0, i]
                    let y2 = finavalu_temp[1, i]
                    in x + h * (b31 * y1 + b32 * y2)
               else if ii == 3
               then let x = initvalu[i]
                    let y1 = finavalu_temp[0, i]
                    let y2 = finavalu_temp[2, i]
                    in x + h * (b41 * y1 + b43 * y2)
               else if ii == 4
               then initvalu[i]
                    + h
                      * (b51 * finavalu_temp[0, i]
                         + b53 * finavalu_temp[2, i]
                         + b54 * finavalu_temp[3, i])
               else if ii == 5
               then initvalu[i]
                    + h
                      * (b61 * finavalu_temp[0, i]
                         + b64 * finavalu_temp[3, i]
                         + b65 * finavalu_temp[4, i])
               else if ii == 6
               then initvalu[i]
                    + h
                      * (b71 * finavalu_temp[0, i]
               ...
               then initvalu[i]
                    + h
                      * (b81 * finavalu_temp[0, i]
                         + b85 * finavalu_temp[4, i]
                         + b86 * finavalu_temp[5, i]
                         + b87 * finavalu_temp[6, i])
               else if ii == 8
               then initvalu[i]
                    + h
                      * (b91 * finavalu_temp[0, i]
                         + b94 * finavalu_temp[3, i]
                         + b95 * finavalu_temp[4, i]
                         + b96 * finavalu_temp[5, i]
                         + b97 * finavalu_temp[6, i]
               ...
               then initvalu[i]
                    ...
                    initvalu[i]
                    + h
                      * (b13_1 * finavalu_temp[0, i]
                         + b13_4 * finavalu_temp[3, i]
                         + b13_5 * finavalu_temp[4, i]
                         + b13_6 * finavalu_temp[5, i]
                         + b13_7 * finavalu_temp[6, i]
                         + b13_8 * finavalu_temp[7, i]
                         + b13_9 * finavalu_temp[8, i]
                         + b13_10 * finavalu_temp[9, i]
                         + finavalu_temp[11, i]))
            (iota equs)
      let timeinst_temp =
        if (ii == 0)
        ...
        then timeinst
             ...
             timeinst + h
      in #[unsafe]
         let finavalu_temp[ii] = master (timeinst_temp, initvalu_temp, parameter)
         in finavalu_temp
  --------------
  -- end loop --
  --------------
...
def solver [pars] [equs] (xmax: i32, params: [pars]f32, y0: [equs]f32) : (bool, [equs]f32) =
  ...
  let y_km1 = y0
  in if xmax < xmin || h <= 0.0f32
     then (false, y_km1)
     else if xmax == xmin
     then (true, y_km1)
     else -- initialize return and loop-variant params
          let failed = false
          let km1 = 0
          let (_, failed, y_km1) =
            loop ((km1, failed, y_km1)) while ((!failed) && (km1 < xmax)) do
              -- for km1 < xmax do
              ...
              let (_, _, y_k, breakLoop, _) =
                loop (j, h, _, breakLoop, _) =
                       (j, h, y_k, breakLoop, scale_fina)
                while ((!breakLoop) && (j < attempts ())) do
                  ...
                  in if (!error)
                     then (j + 1, h, y_k, true, max_scale_factor ())
                     else -- FiGURE OUT SCALE AS THE MiNiMUM OF COMPONENT SCALES
                          let yy =
                            map (\(x: f32) : f32 ->
                                   if (x == 0.0f32) then tolerance else fabs (x))
                                (y_km1)
                          let scale =
                            map (\(yy_erri: (f32, f32)) : f32 ->
                          ...
                          let h =
                            if (f32.i32 (km1) + h > f32.i32 (xmax))
                            then f32.i32 (xmax - km1)
...
def main (repeat: i32)
         ...
         (xmax: i32)
         (y0: [91]f32)
         (params: [16]f32) : (bool, [workload][91]f32) =
  let (oks, y_res) =
    unzip
    <| #[sequential_inner]
       tabulate workload
