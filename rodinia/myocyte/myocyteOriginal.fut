...
fun *[f32,EQUS] master( f32 timeinst, [f32,EQUS] initvalu, [f32,PARS] parameter ) =
  ...
  let initvalu_offset_Dyad  = 46 in
  let parameter_offset_Dyad = 1  in
  let CaDyad = initvalu[35]*1e3f32  in -- from ECC model, *** Converting from [mM] to [uM] ***
  let {JCaDyad, finavalu} = cam(    timeinst, initvalu, initvalu_offset_Dyad, parameter,
                                    parameter_offset_Dyad, finavalu, CaDyad )
  in
  -- cam function for SL
  let initvalu_offset_SL  = 61 in
  let parameter_offset_SL = 6  in
  let CaSL = initvalu[36]*1e3f32  in -- from ECC model, *** Converting from [mM] to [uM] ***
  let {JCaSL, finavalu} = cam(  timeinst, initvalu, initvalu_offset_SL, parameter,
                                parameter_offset_SL, finavalu, CaSL ) 
  in
  -- cam function for Cyt
  let initvalu_offset_Cyt  = 76 in
  let parameter_offset_Cyt = 11 in
  let CaCyt = initvalu[37]*1e3f32  in -- from ECC model, *** Converting from [mM] to [uM] ***
  let {JCaCyt, finavalu} = cam(	timeinst, initvalu, initvalu_offset_Cyt, parameter,
                                parameter_offset_Cyt, finavalu, CaCyt   )
  in
  -- final adjustments
  let finavalu = fin(  initvalu, initvalu_offset_ecc, initvalu_offset_Dyad, initvalu_offset_SL,
...
embedded_fehlberg_7_8(  f32 timeinst, f32 h,
  ...
  let initvalu_temp = initvalu in -- copy(initvalu) ???
  let finavalu_temp[0] = master( timeinst_temp, initvalu_temp, parameter ) 
  in
  ---------
  --- 2 ---
  ---------
  ...
  let initvalu_temp = map( fn f32 (int i) => 
                                initvalu[i] + h * ( b51*finavalu_temp[0,i] + b53*finavalu_temp[2,i] + b54*finavalu_temp[3,i])
                         , iota(EQUS) ) in
