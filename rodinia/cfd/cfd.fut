...
def var_density_energy : i32 = var_momentum + 3
...
--
def initialize_variables (nelr: i64) (ff_variable: [5]f32) : [5][nelr]f32 =
  --[#nvar]float ff_variable
  map (\(x: f32) : [nelr]f32 -> replicate nelr x) (ff_variable)

--
def compute_flux_contribution ( _density: f32
...
def compute_step_factor [nelr]
                        (variables: [5][nelr]f32, areas: [nelr]f32) : [nelr]f32 =
  -- 5 == nvar
  map2 (\area variables' ->
...
def compute_flux [nnb] [nel] [ndim]
  ...
  in transpose (map3 (\variables' elements_surrounding_elements' normals' : [5]f32 ->
                        ...
                        let ( flux_contribution_i_momentum_x
                        ...
                        let ( flux_contribution_i_momentum_x_x
                            , flux_contribution_i_momentum_x_y
                            , flux_contribution_i_momentum_x_z
                            ) =
                          flux_contribution_i_momentum_x
                        let ( flux_contribution_i_momentum_y_x
                            , flux_contribution_i_momentum_y_y
                            , flux_contribution_i_momentum_y_z
                            ) =
                          flux_contribution_i_momentum_y
                        let ( flux_contribution_i_momentum_z_x
                            , flux_contribution_i_momentum_z_y
                            , flux_contribution_i_momentum_z_z
                            ) =
                          flux_contribution_i_momentum_z
                        let ( flux_contribution_i_density_energy_x
                            , flux_contribution_i_density_energy_y
                        ...
                        let loop_res =
                          ...
                          for (nb, normal_x, normal_y, normal_z) in zip4 elements_surrounding_elements'
                            ...
                            in if (0 <= nb)
                               then -- a legitimate neighbor
                                    let density_nb = #[unsafe] variables[var_density, nb]
                                    ...
                                    let ( flux_contribution_nb_momentum_x
                                    ...
                                    let ( flux_contribution_nb_density_energy_x
                                        , flux_contribution_nb_density_energy_y
                                        , flux_contribution_nb_density_energy_z
                                        ) =
                                      flux_contribution_nb_density_energy
                                    let ( flux_contribution_nb_momentum_x_x
                                        , flux_contribution_nb_momentum_x_y
                                        , flux_contribution_nb_momentum_x_z
                                        ) =
                                      flux_contribution_nb_momentum_x
                                    let ( flux_contribution_nb_momentum_y_x
                                        , flux_contribution_nb_momentum_y_y
                                        , flux_contribution_nb_momentum_y_z
                                        ) =
                                      flux_contribution_nb_momentum_y
                                    let ( flux_contribution_nb_momentum_z_x
                                        , flux_contribution_nb_momentum_z_y
                                        , flux_contribution_nb_momentum_z_z
                                        ) =
                                      flux_contribution_nb_momentum_z
                                    -- artificial viscosity
                                    let factor =
                                      -normal_len * smoothing_coefficient * 0.5
...
def time_step [nel]
  ...
  transpose (map3 (\old_variables' fluxes' step_factor : [5]f32 ->
...
def main [nel]
  ...
  let (ff_velocity_x, ff_velocity_y, ff_velocity_z) =
    ( ff_speed * f32.cos (angle_of_attack)
    , -- .x   ... cos(angle_of_attack) = 1
      ff_speed * f32.sin (angle_of_attack)
    , -- .y   ... sin(angle_of_attack) = 0
      0.0f32
    )
  -- .z
  ...
  let ( ff_flux_contribution_momentum_x
      , ff_flux_contribution_momentum_y
      , ff_flux_contribution_momentum_z
      , ff_flux_contribution_density_energy
      ) =
  ...
  in loop (variables) for _i < iterations do
       ...
       let fluxes =
         compute_flux ( elements_surrounding_elements
                      , normals
                      , variables
                      , ff_variable
                      , ff_flux_contribution_momentum_x
