...
type~ simulation_data =
  ?[max_windows][max_poles][length_num_nucs][n_nuclides][numL][num_mats][max_num_nucs].{ n_windows: [n_nuclides]i32
...
def fast_forward_LCG (seed: seed) (n: i64) : seed =
  ...
  let (_, _, _, a_new, c_new) =
    loop (n, a, c, a_new, c_new) = (n, a, c, 1, 0)
    for _i < 64 - u64.clz n do
      let (a_new, c_new) =
        if n & 1 == 1
...
def calculate_sig_T (nuc: i32) (E: f64) (data: simulation_data) : [4]rs_complex =
  let f i =
    let phi = data.pseudo_K0RS[nuc, i] * f64.sqrt E
    let phi =
      match i
      case 1 -> phi - -(f64.atan phi)
...
def fast_nuclear_W (Z: rs_complex) : rs_complex =
  ...
  then let prefactor = {r = 0, i = 8.124330e+01}
  ...
  else let a = {r = 0.512424224754768462984202823134979415014943561548661637413182, i = 0}
       ...
       let W =
         (Z `c_mul` i)
         `c_mul` ((a `c_div` (Z2 `c_sub` b)) `c_add` (c `c_div` (Z2 `c_sub` d)))
       in W

def calculate_micro_xs_doppler (nuc: i32) (E: f64) (data: simulation_data) : (f64, f64, f64, f64) =
...
def calculate_micro_xs (nuc: i32) (E: f64) (data: simulation_data) : (f64, f64, f64, f64) =
  let spacing = 1 / f64.i32 data.n_windows[nuc]
  let window = i32.min (data.n_windows[nuc] - 1) (i32.f64 (E / spacing))
  let sigTfactors = calculate_sig_T nuc E data
  let w = data.windows[nuc, window]
...
def calculate_macro_xs (mat: i32)
                       (E: f64)
                       (doppler: i32)
                       (data: simulation_data) : (f64, f64, f64, f64) =
  loop macro_xs = (0, 0, 0, 0)
  for i < data.num_nucs[mat] do
...
def argmax [n] (xs: [n]f64) : i64 =
  let max (x1, y1) (x2, y2) = if y1 < y2 then (x2, y2) else (x1, y1)
...
def unpack lookups
  ...
  let unpack_window (f64s: [3]f64) (i32s: [2]i32): window = {T = f64s[0], A = f64s[1], F = f64s[2], start = i32s[0], end = i32s[1]}
...
entry diff lookups
  ...
  let (input, sd) =
    unpack lookups
           ...
           concs
  let diff_res =
    (vjp (run_event_based_simulation input.lookups input.doppler)
         sd
