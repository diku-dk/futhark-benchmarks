...
module lys : lys with text_content = text_content = {
  ...
  def event (e: event) (s: state) =
    ...
    case #mouse {buttons, x, y} ->
      s with mouse = (x, y)
        with config = if buttons != 0
        then move s (diff s.mouse (x, y))
        else s.config
    case #wheel {dx = _, dy} ->
  ...
  def text_content (fps: f32) (s: state) =
    ( i32.f32 (f32.round fps)
    , s.precision
    , s.config.limit
    , f32.f64 s.config.radius
    )
  ...
  def render (s: state) =
    if s.precision == 32
    then mandelbrot32.render_mandelbrot s.width
                                        s.height
                                        s.config.xcentre
                                        s.config.ycentre
                                        s.config.width
                                        s.config.limit
                                        s.config.radius
    else mandelbrot64.render_mandelbrot s.width
                                        s.height
                                        s.config.xcentre
                                        s.config.ycentre
                                        s.config.width
