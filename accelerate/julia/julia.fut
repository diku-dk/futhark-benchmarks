...
def julia (screenX: i64)
          ...
          (width: f32)
          (limit: i32)
          (radius: f32) : [screenY][screenX](c32, i32) =
...
def render (palette: []argb.colour)
           ...
           (width: f32)
           (limit: i32)
           (radius: f32) : [screenY][screenX]argb.colour =
...
def ultra (p: f32) =
  ...
  let p4 = 0.8575
  let p5 = 1.0
  let rgb8 (r: i32) (g: i32) (b: i32) = argb.from_rgba (f32.i32 r / 255.0) (f32.i32 g / 255.0) (f32.i32 b / 255.0) 0.0
  let c0 = rgb8 0 7 100
  ...
  let m5 = m0
  in if p <= p1
     then interp (p0, p1) (c0, c1) (m0, m1) p
     else if p <= p2
     then interp (p1, p2) (c1, c2) (m1, m2) p
     else if p <= p3
     then interp (p2, p3) (c2, c3) (m2, m3) p
     else if p <= p4
     then interp (p3, p4) (c3, c4) (m3, m4) p
     else interp (p4, p5) (c4, c5) (m4, m5) p
...
module lys : lys with text_content = () = {
  ...
  def event (e: event) (s: state) =
    ...
    case #keydown {key} ->
      ...
      then load_preset s presets[key - SDLK_1]
      else if key == SDLK_z
      then s with radius = s.radius * 0.5
      else if key == SDLK_c
      then s with radius = s.radius * 2
      else if key == SDLK_a
      then s with iters = i32.f32 (f32.i32 s.iters * 0.8)
      else if key == SDLK_d
      then s with iters = i32.f32 (f32.i32 s.iters * 1.2)
      else if key == SDLK_q
      then s with speed = s.speed * 0.75
      else if key == SDLK_e
      then s with speed = s.speed * 1.25
      else if key == SDLK_w
      then s with zooming = -0.25
      else if key == SDLK_s
      then s with zooming = 0.25
      else if key == SDLK_RIGHT
      then s with time = s.time + s.speed / 60
      else if key == SDLK_LEFT
      then s with time = s.time - s.speed / 60
      else if key == SDLK_SPACE
      then s with animating = !s.animating
      else s
    case #keyup {key} ->
      if key == SDLK_w || key == SDLK_s
      then s with zooming = 0
      else s
