...
def cast_view_rays (sizeX: i64) (sizeY: i64) (fov: i32) (eye_dir: position) : [sizeY][sizeX]direction =
...
def hit_sphere (sph: sphere) (dist: f32) (orig: position) (dir: direction) : (position, direction, argb.colour, f32) =
...
def hit_plane (pln: plane) (dist: f32) (orig: position) (dir: direction) : (position, direction, argb.colour, f32) =
  let point = orig vec3.+ vec3.scale dist dir
...
def trace_ray (limit: i32)
              ({spheres, planes}: objects [] [])
              (lights: lights [])
              (ambient: argb.colour)
              (orig_point: position)
              (orig_dir: direction) =
  ...
  let (_, refl_colour, _, _, _) =
    loop (i, refl_colour, point, dir, visibility) =
           (0, argb.black, orig_point, orig_dir, 1.0)
    while i < limit do
      ...
      in if !(hit_s || hit_p)
         then (limit, refl_colour, point, dir, visibility)
...
def make_objects (time: f32) : objects [] [] =
  { spheres =
      [ { position = {x = 40.0 * f32.sin time, y = -80.0, z = 0.0}
        , radius = 20.0
        , colour = argb.from_rgba 1.0 0.3 1.0 1.0
        , shine = 0.4
        }
      , { position =
            { x = 200.0 * f32.sin time
...
def main (sizeX: i32)
  ...
  let lights =
    { lights =
        [ { position = {x = 250.0, y = 350.0, z = 200.0}
          , colour = argb.red
          }
        ]
    }
  ...
  let eye_dir =
    { x = f32.cos eye_dir_A * f32.cos eye_dir_B
    , y = f32.sin eye_dir_B
