...
module lys : lys with text_content = text_content = {
  ...
  def keydown (k: i32) (s: state) =
    if k == 'w'
    then s with forward_sgn = 1
    else if k == 's'
    then s with forward_sgn = -1
    else if k == 'a'
    then s with sideway_sgn = -1
    else if k == 'd'
    then s with sideway_sgn = 1
    else if k == 'z'
    then s with limit = i32.max (s.limit - 1) 0
    else if k == 'x'
    then s with limit = s.limit + 1
    else s

  def keyup (k: i32) (s: state) =
    if k == 'w'
    then s with forward_sgn = 0
    else if k == 's'
    then s with forward_sgn = 0
    else if k == 'a'
    then s with sideway_sgn = 0
    else if k == 'd'
    then s with sideway_sgn = 0
    else s
  ...
  def event (e: event) (s: state) =
    ...
    case #mouse {buttons, x, y} ->
      if buttons == 0
      then s with eye.a = s.eye.a + f32.i32 x / f32.i64 s.width
