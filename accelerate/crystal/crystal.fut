...
def point (scale: f32, x: f32, y: f32) : (f32, f32) =
  (x * scale, y * scale)

def rampColour (v: f32) : argb.colour =
...
def wave (th: f32, x: f32, y: f32) : f32 =
  let cth = f32.cos (th)
  let sth = f32.sin (th)
  in (f32.cos (cth * x + sth * y) + 1.0) / 2.0

def waver (th: f32, x: f32, y: f32, n: i64) : f32 =
  f32.sum (tabulate n (\i -> wave (f32.i64 (i) * th, x, y)))

def waves (degree: i64, phi: f32, x: f32, y: f32) : f32 =
  let th = f32.pi / phi
  in wrap (waver (th, x, y, degree))

def quasicrystal (scale: f32, degree: i64, time: f32, x: f32, y: f32) : argb.colour =
  let phi = 1.0 + (time ** 1.5) * 0.005
  let (x', y') = point (scale, x, y)
  in rampColour (waves (degree, phi, x', y'))

def normalize_index (i: i64, field_size: i64) : f32 =
  f32.i64 (i) / f32.i64 (field_size)

entry render_frame (field_size: i64) (scale: f32) (degree: i64) (time: f32) : [field_size][field_size]argb.colour =
  tabulate_2d field_size
              field_size
              (\y x ->
                 quasicrystal ( scale
                              , degree
...
entry main (field_size: i32)
  ...
  let frames =
    tabulate (i64.i32 n_steps) (\step_i ->
