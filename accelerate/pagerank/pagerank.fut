...
def calculate_page_ranks [n] (links: []link) (ranks: *[n]f32) (sizes: [n]i32): *[n]f32 =
  ...
  let page_contributions = scanned_contributions
  let page_tos =
    map2 (\to flag -> if flag then i64.i32 to else -1)
         tos (rotate 1 page_flags)
...
entry preprocess_graph [m] (links_array: [m][2]i32): ([m]i32, [m]i32, []i32) =
  ...
  let n = i32.maximum (1 + (.from) links_by_to)
  in ((.from) links_by_to,
      (.to) links_by_to,
