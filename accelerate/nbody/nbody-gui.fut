...
def render_point (h: i64)
                 (w: i64)
                 {x = x_ul: f32, y = y_ul: f32}
                 {x = x_br: f32, y = y_br: f32}
                 (max_mass: f32)
                 ({x, y, z = _}: position)
                 (m: f32) : (i64, argb.colour) =
  -- Draw nothing if the point is outside the viewport.
  if x < x_ul || x > x_br || y < y_ul || y > y_br
  then (-1, 0)
  else -- Normalise x,y to positions in interval (0,1) within the viewport.
...
def mk_bodies (mk: (rnge.rng -> (rnge.rng, body))) (rng: rnge.rng) (n: i64) : (rnge.rng, [n]body) =
...
module lys : lys with text_content = info = {
  ...
  def blob x y (s: state) : state =
    ...
    let blob_N = 100
    let move (body: body) =
      body with position.x = body.position.x + x
  ...
  def maybe_attract [n] (opt_attractor: opt vec3.vector) (td: f32) (bodies: [n]body) : [n]body =
    match opt_attractor
    case #none -> bodies
    case #some position ->
  ...
  def move_bodies td (s: state) =
    s with bodies = if s.paused
      then s.bodies
      else maybe_attract s.attractor
                         td
                         (match s.solver
                          case #bruteforce ->
  ...
  def event (e: event) (s: state) =
    ...
    case #keydown {key} ->
      if key == SDLK_RIGHT
      then s with rotating.y = 1
      else if key == SDLK_LEFT
      then s with rotating.y = -1
      else if key == SDLK_UP
      then s with rotating.x = 1
      else if key == SDLK_DOWN
      then s with rotating.x = -1
      else if key == SDLK_SPACE
      then s with paused = !s.paused
      else if key == SDLK_PAGEUP
      then s with zooming = 1
      else if key == SDLK_PAGEDOWN
      then s with zooming = -1
      else if key == SDLK_m
      then s with solver = match s.solver
             case #bruteforce -> #bh
             case #bh -> #bruteforce
      else if key == SDLK_1
      then s with theta = f32.max 0 (s.theta * 0.9)
      else if key == SDLK_2
      then s with theta = f32.min 100 (s.theta * 1.1)
      else if key == SDLK_s
      then let radius =
    ...
    case #keyup {key} ->
      ...
      then s with rotating.y = 0
      else if key == SDLK_UP || key == SDLK_DOWN
      then s with rotating.x = 0
  ...
  def render (s: state) =
    ...
    let (is, vs) =
      unzip (map2 (render_point s.height s.width s.ul s.br s.max_mass)
