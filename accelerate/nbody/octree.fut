  ...
  let octree =
    map2 (\i rp ->
            ...
            let tree_level =
              if i == 0
              then 0
              else node_level + (if is_left then left_edge else edges'.right)
            let child =
              if is_left
              then radix_parent.left
            ...
            let body =
              if is_leaf
              then --- if we are a leaf, child will be a pointer to
            ...
            let parent =
              ...
              then -1
                   ...
                   let parent' = -1
                   let (parent', _) =
                     loop (parent', ridx) while parent' == -1 do
                       -- O(3)
                       let parent_ridx = inners[ridx].parent
                       in if parent_ridx == -1
                          then -- octree root is parent
                               (0, parent_ridx)
                          else -- we checked parent_ridx != -1, so it must
            ...
            let ne: i32 = -1
            let children =
              let arr = replicate 8 ne
              in if is_leaf
                 then arr
                 else let ridx = if i == 0 then 0 else unpack child
                      let lvl: i32 = 0
                      let to_check = [2, 0, 0]
                      let child_idx: i32 = 0
                      let (arr', _, _, _, _) =
                        ...
                        while to_check[0] > 0 do
                          ...
                          then -- check left edge
                               if edges[ridx].left != 0
                               then let start = ranges[ridx]
                                    ...
                                    let to_check[lvl] = 2
                                    in (arr, to_check, child_idx, ridx, lvl)
                          else if to_check[lvl] == 1
                          then -- check right edge
                               if edges[ridx].right != 0
                               then let start = ranges[ridx]
                                    let arr[child_idx] = start + (i32.bool (edges[ridx].left > 0)) * 1
                                    let child_idx = child_idx + 1
                                    let to_check[lvl] = 0
                                    in (arr, to_check, child_idx, ridx, lvl)
                               else -- move down one level
                                    ...
                                    let to_check[lvl] = 2
                                    in (arr, to_check, child_idx, ridx, lvl)
                          else let ridx = inners[ridx].parent
                               let lvl = lvl - 1
