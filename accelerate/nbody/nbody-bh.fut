...
def move [n]
         (epsilon: f32)
         (theta: f32)
         (t: [n]octnode, side_max: f32, root_delta: i32)
         (x: pointmass) : acceleration =
  let (acceleration, _, _, _) =
    ...
    while to_check > 0 do
      ...
      let current = t[cur]
      in if from_child
         then -- prev is my child => just move to next node
              --- index 'i' of child will be found before 'i' goes out of bounds.
              let j =
                loop i = 0
                while (current.children)[i] != prev do
                  (i + 1)
              let next_cur =
                if j == 7 || ((current.children)[j + 1]) == -1
                then current.parent
                else (current.children)[j + 1]
              --- j must be less than 7
              ...
              in if current.is_leaf || overlapping_subtree || local_theta < theta
                 then let velocity = accel epsilon x {position = q, mass = total_mass}
...
def calc_accels [n] (epsilon: f32) (theta: f32) (bodies: [n]body) : ([n]acceleration, [n]body) =
