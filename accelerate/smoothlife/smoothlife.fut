...
def generate_world (size: i64) (disc: (f32, f32)) (seed: i32) : [size][size]f32 =
  ...
  let points_in_line (_, _, x1, y1, x2, y2): i64 = i64.i32 (i32.(1 + max (abs (x2 - x1)) (abs (y2 - y1))))
  let get_point_in_line (z, cell, x1, y1, x2, y2) (i: i64) =
    let i = i32.i64 i
    let compare (v1) (v2) = if v2 > v1 then 1 else if v1 > v2 then -1 else 0
    ...
    let p1 = (x1, y1)
    let p2 = (x2, y2)
    in if i32.abs (p1.0 - p2.0) > i32.abs (p1.1 - p2.1)
...
def apply_sigmoid (s: sigmoid) (x: f32) (a: f32) (ea: f32) =
  ...
  in match s
     ...
     case #atancos ->
       0.5
       * (0.5 * f32.atan ((x - a) / ea) / f32.pi
          * f32.cos ((x - a) * 1.4)
          * 1.1
          + 1.0)
     case #overshoot ->
       0.5
       + (1.0 / (1.0 + f32.exp (-(x - a) * 4.0 / ea)) - 0.5)
...
def step [size] (state: state [size]) : state [size] =
  ...
  let aa' =
    snm state.conf (take (size * size) n) (take (size * size) m)
    |> unflatten
  let timestep f g =
