...
def grid_search [n] (quarry: f64) (A: [n]f64) : i64 =
  ...
  let upperLimit = n - 1
  in (.0)
     <| loop (lowerLimit, upperLimit) for _i < 64 - i64.clz n do
...
def calculate_macro_xs [n_isotopes] [n_gridpoints] [max_num_nucs]
  ...
  let idx =
    ...
    case #nuclide ->
      -1
  in loop macro_xs_vector = (0, 0, 0, 0, 0)
     for j < num_nucs[mat] do
...
def run_event_based_simulation [length_num_nucs]
                               ...
                               [n_gridpoints]
                               (inp: inputs)
                               (sd: simulation_data [length_num_nucs] [length_unionized_energy_array] [max_num_nucs] [n_isotopes] [n_gridpoints]
                               ) =
  let f i =
    let seed = fast_forward_LCG STARTING_SEED (2 * i)
    let (p_energy, seed) = LCG_random_double seed
    let (mat, _seed) = pick_mat seed
    in calculate_macro_xs p_energy
                          mat
                          sd.num_nucs
                          sd.concs
...
def unpack n_isotopes
           ...
           nuclide_grid
           index_grid
           unionized_energy_array : (inputs, simulation_data [] [] [] [] []) =
  let grid_type =
    match grid_type : i64
    case 0 -> #unionized
    case 1 -> #nuclide
