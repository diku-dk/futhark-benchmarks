...
def plus_scan [n] (x: [n]i32): [n]i32 =
  scan (+) 0 x

def plus_prescan [n] (x: [n]i32): [n]i32 =
  let xshifted = map (\i  -> if i == 0 then 0 else x[i - 1]) (iota n)
  in scan (+) 0 xshifted

def permute [n] (a: [n]u32, index: [n]i32): [n]u32 =
  scatter (copy a) (map i64.i32 index) a

def plus_scan_reverse_order [n] (x: [n]i32): [n]i32 =
  let xreversed = reverse x
  let x' = plus_scan xreversed
  let x'reversed = reverse x'
  in x'reversed

def split_blelloch [n] (a: [n]u32, flags: [n]i32): [n]u32 =
...
def split_radix_sort [n] (a: [n]u32, number_of_bits: i32): [n]u32 =
