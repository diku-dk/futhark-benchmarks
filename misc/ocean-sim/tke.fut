  ...
  map2 (\square i ->
          map2 (\row j ->
                  map2 (\el k -> f el i j k)
                       row
                       (map i32.i64 (iota zdim)))
               square
               (map i32.i64 (iota ydim)))
       arr
       (map i32.i64 (iota xdim))
...
entry main [xdim] [ydim] [zdim]
           ...
           (forc_tke_surface: [xdim][ydim]DTYPE
           ) =
  --  :
  -- (tke: *[3][h][w][d]DTYPE,
  -- dtke: *[3][h][w][d]DTYPE,
  ...
  -- Init delta
  let delta =
    tabulate_3d xdim ydim zdim (\x y z ->
  ...
  let b_tri =
    map3D tketau
          (\el_tketau x y z ->
             let sqrttke = real_sqrt (real_max 0 el_tketau)
             in if x >= 2i32 && x < (i32.i64 xdim) - 2i32 && y >= 2i32 && y < (i32.i64 ydim) - 2i32
                then let ks_val = kbot[x, y] - 1
                     let land_mask = ks_val >= 0
                     ...
                     in if !water_mask
                        ...
                        else if z > 0i32 && z < (i32.i64 zdim) - 1i32
  ...
  let d_tri =
    tabulate_3d xdim ydim zdim (\x y z ->
                                  ...
                                  then let ks_val = kbot[x, y] - 1
                                       ...
                                       in if !water_mask
                                          then 0
                                          else let tmp = tketau[x, y, z] + dt_tke * forc[x, y, z]
                                               in if z == zdim - 1
  ...
  let tketaup1 =
    tabulate_3d xdim ydim zdim (\x y z ->
                                  let ks_val = kbot[x, y] - 1
                                  let water_mask = (ks_val >= 0) && ((i32.i64 z) >= ks_val)
                                  in if x >= 2 && x < xdim - 2 && y >= 2 && y < ydim - 2 && water_mask
                                     then sol[x, y, z]
                                     else tketaup1[x, y, z])
  let tke_surf_corr =
    tabulate_2d xdim ydim (\x y ->
                             if x >= 2 && x < xdim - 2 && y >= 2 && y < ydim - 2
                             then let tke_val = tketaup1[x, y, zdim - 1]
                                  in if tke_val < 0
  ...
  let tketaup1 =
    tabulate_3d xdim ydim zdim (\x y z ->
                                  if x >= 2 && x < xdim - 2 && y >= 2 && y < ydim - 2 && z == zdim - 1
                                  then let tke_val = tketaup1[x, y, z]
                                       in if tke_val < 0
                                          then 0
                                          else tke_val
                                  else tketaup1[x, y, z])
  ...
  let flux_east =
    tabulate_3d xdim ydim zdim (\x y z ->
                                  if x < xdim - 1
                                  then K_h_tke * (tketau[x + 1, y, z] - tketau[x, y, z])
                                       / (cost[y] * dxu[x])
                                       * maskU[x, y, z]
                                  else 0)
  -- lateral diff north
  let flux_north =
    tabulate_3d xdim ydim zdim (\x y z ->
                                  if y < ydim - 1
                                  then K_h_tke * (tketau[x, y + 1, z] - tketau[x, y, z])
                                       / dyu[y]
                                       * maskV[x, y, z]
                                       * cosu[y]
                                  else 0)
  ...
  let tketaup1 =
    tabulate_3d xdim ydim zdim (\x y z ->
                                  let previous = tketaup1[x, y, z]
                                  in if x >= 2 && x < xdim - 2 && y >= 2 && y < ydim - 2 && z == zdim - 1
                                     then previous
                                          + dt_tke * maskW[x, y, z]
  ...
  let dtketau =
    tabulate_3d xdim ydim zdim (\x y z ->
                                  ...
                                  in if z == zdim - 1
                                     then z_middle_update
                                          - (flux_top[x, y, z] - flux_top[x, y, z - 1])
                                            / (0.5 * dzw[z])
