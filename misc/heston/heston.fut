...
module heston
  ...
  def run_calibration ({ today
    ...
    let prices_and_vegas =
      map (\{maturity, strike, quote} ->
             price_and_vega_of_quote strike maturity quote)
          quotes
    ...
    let objective (x: []real): real =
      ...
      let x_prices =
        pricer.price_european_calls gauss_laguerre_coefficients
                                    false
                                    (int 1)
                                    (int 1)
                                    (int 1)
      ...
      in real_distance.distance (map2 (\(p, v) w -> real.(w * p / v)) prices_and_vegas weights)
                                (map3 (\w (_, v) p -> real.(w * p / v)) weights prices_and_vegas x_prices)
    in real_least_squares.least_squares variables
                                        objective
                                        max_global
                                        np
  ...
  def default_variables : [5]real_least_squares.optimization_variable =
    [ real_least_squares.optimize_value {lower_bound = real.f64 1e-6, initial_value = real.f64 4e-2, upper_bound = int 1}
    , real_least_squares.optimize_value {lower_bound = real.f64 1e-6, initial_value = real.f64 4e-2, upper_bound = int 1}
    , real_least_squares.optimize_value {lower_bound = int (-1), initial_value = real.f64 (-0.5), upper_bound = int 0}
    , real_least_squares.optimize_value {lower_bound = real.f64 1e-4, initial_value = real.f64 1e-2, upper_bound = int 4}
    , real_least_squares.optimize_value {lower_bound = real.f64 1e-4, initial_value = real.f64 0.4, upper_bound = int 2}
    ]
  ...
  def heston [num_quotes]
    ...
    let result =
      run_calibration { today = date_of_int today
                      , quotes =
                          map3 (\m k q -> {maturity = date_of_int m, strike = k, quote = q})
                               quotes_maturity
                               quotes_strike
                               quotes_quote
    ...
    let { initial_variance
      ...
      heston_parameters_from_vector result.parameters
