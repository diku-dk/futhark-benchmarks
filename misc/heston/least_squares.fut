...
module type least_squares = {
  ...
  type calibration_result [num_vars] =
    { parameters: [num_vars]real
...
module mk_least_squares (real: real) (rand: rng_engine)
  ...
  def optimize [num_vars] [num_free_vars]
    ...
    let rng = rngs[0]
    let x =
      (let init_j (lower_bound: real) (upper_bound: real) (r: real) = real.(lower_bound + (upper_bound - lower_bound) * r)
    ...
    let mutation (difw: real) (best_idx: i32) (x: [np][num_free_vars]real) (rng: rand.rng) (i: i32) (x_i: [num_free_vars]real) =
      (-- We have to draw 'to_draw' distinct elements from 'x', and it
       ...
       let v_i =
         map5 (\j r (lower_bound, upper_bound) aux x_i_j ->
              ...
              x_i
       in (rng, v_i))
    let recombination (fx0: real) (best_idx: i32) (fx: [np]real) (x: [np][num_free_vars]real) (v: [np][num_free_vars]real) =
      (let f_v = map objective' v
       let fx' = map2 real.min f_v fx
       let x' =
         map4 (\f fx_i x_i v_i -> real.(if f < fx_i then v_i else x_i))
              f_v
              fx
              x
              v
    ...
    let (_, ncalls, _num_it, (_, _, _, x)) =
      ...
      while i32.(num_it > 0) && (max_global > ncalls) && (fx0 real.> target) do
        (let (rng, differential_weight) = random_real.rand (real.from_fraction 1 2, real.i32 1) rng
         let rngs = rand.split_rng np rng
         let (rngs, v) =
           unzip (map3 (mutation differential_weight best_idx x)
                       rngs
                       (map i32.i64 (iota np))
                       x)
         let rng = rngs[0]
    ...
    let status =
      if real.(fx0 <= target)
      then #target_reached
      else if i32.(max_global < ncalls)
      then #max_global_reached
    ...
    let (x, num_feval) =
      ...
      then let res =
             (optimize objective
                       vars_to_free_vars
                       variables
                       {np = i64.i32 np, cr = real.from_fraction 9 10}
                       lower_bounds
                       upper_bounds
