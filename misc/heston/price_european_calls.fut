...
type heston_parameters 'real =
  { initial_variance: real
...
module price_european_calls (R: real)
  ...
  def ugaussian_P (x: real) =
    if R.isinf x
    then (if x > int 0 then int 1 else int 0)
    else int 1 - int 0 * erfc (x * inv_sqrt2)

  def gauss_laguerre_coefficients (nb: num_points) =
    match nb
    case #ten ->
      zip (map real [ 0.1377934705404924298211
                    , 0.7294545495031707904587
                    , 1.8083429017403143124199
                    , 3.4014336978548351808627
                    , 5.5524961400642398601235
                    , 8.3301527467632645596041
                    , 11.8437858379019154142497
                    , 16.2792578313766149733510
                    , 21.9965858119813617577165
                    , 29.9206970122737736517138
                    ])
          (map real [ 0.3540097386069980256451
                    , 0.8319023010435621090508
                    , 1.3302885617494675241090
                    , 1.8630639031111377867944
                    , 2.4502555580731559814467
                    , 3.1227641551735070279960
                    , 3.9341526954948387029276
                    , 4.9924148722151180379569
                    , 6.5722024851118048260901
                    , 9.7846958403678012672344
                    ])
    case #twenty ->
      zip (map real [ 0.070539889691988738596
                    , 0.372126818001613290932
                    , 0.916582102483245675373
                    , 1.707306531028168317121
                    , 2.749199255315394108123
                    , 4.048925313808060089116
                    , 5.615174970938836551682
                    , 7.459017454225468135576
                    , 9.594392865483230892210
                    , 12.038802560608859337776
                    , 14.814293416061961039532
                    , 17.948895543122549867121
                    , 21.478788273773705697067
                    , 25.451702644185488111361
                    , 29.932554890200286479285
                    , 35.013433968980073984767
                    , 40.833057239401291838021
                    , 47.619993970772064528774
                    , 55.810795768051811194255
                    , 66.524416523865880890298
                    ])
          (map real [ 0.18108006241898921829
                    , 0.42255676787860191324
                    , 0.66690954670145563554
                    , 0.91535237278121706073
                    , 1.16953970728044676086
                    , 1.43135498604326039107
                    , 1.70298113670862205637
                    , 1.98701590220746626692
                    , 2.28663572001997383865
                    , 2.60583491316971160856
                    , 2.94978326190603512558
                    , 3.32539692360021144069
                    , 3.74225473838238897883
                    , 4.21423782504235067137
                    , 4.76251619189997654757
                    , 5.42172741864077067930
                    , 6.25401126576962873571
                    , 7.38731454069754001068
                    , 9.15132857271978572555
                    , 12.89338863845354232751
                    ])
  ...
  def price_european_calls [num_points] [num_maturities] [num_quotes]
                           ...
                           (quotes: [num_quotes]{strike: real, maturity: i32}) : [num_quotes]real =
  ...
  def bs_call (call: bool) (today: date) (spot: real) (strike: real) (maturity: date) (vol: real) =
    if same_date today maturity || vol <= real 1e-15
    then let forward = spot
         let p = R.max (int 0) (forward - strike)
         in (p, int 0)
    else let normal_dist (x: real) = pnorm x
         let eps = if call then int 1 else int (-1)
         let t = real (diff_dates today maturity)
         let sqrt_t = R.sqrt t
         let df_r = int 1
         let df_d = int 1
         let fwd = spot * df_d / df_r
         let (d1, d2) =
           let d (add: real) = (R.log (fwd / strike) + add * real 0.5 * vol * vol * t) / (vol * sqrt_t)
           in (d (int 1), d (int (-1)))
         let (n1, n2) = (normal_dist (eps * d1), normal_dist (eps * d2))
         in ( eps * df_r * (fwd * n1 - strike * n2)
