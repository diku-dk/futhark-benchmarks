...
def trajectories (rng: rng)
                 (limit: i64)
                 (radius: f32)
                 (npoints: i64)
                 (field: (f32, f32, f32, f32)) : (rng, []([limit]c32, bool)) =
  let (xmin, ymin, xmax, ymax) = field
  let mk_point rng =
    let (rng, x) = dist.rand (xmin, xmax) rng
...
def visualise (screenX: i64)
              (screenY: i64)
              (state: state [screenY * screenX])
              (limit: i64)
              (radius: f32)
              (npoints: i64)
              (field: (f32, f32, f32, f32)) : (state [], [screenY][screenX]argb.colour) =
...
entry main (screenX: i64)
           ...
           limit
           radius
           npoints : (state [], [screenY][screenX]argb.colour) =
