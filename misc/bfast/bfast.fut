...
def gauss_jordan [nm] (n: i64) (A: *[nm]f32) : [nm]f32 =
  let m = nm / n
  in loop A for i < n do
       let v1 = #[unsafe] A[i]
       let A' =
         map (\ind ->
                let (k, j) = (ind / i32.i64 m, ind % i32.i64 m)
                let x = #[unsafe] (A[j] / v1)
                in if k < i32.i64 n - 1
                   then -- Ap case
                        #[unsafe]
...
def bfast [N]
  ...
  let Xsqr = matsqr flgsh Xh
  -- [2k+2][2k+2]
  let Xinv = mat_inv Xsqr
  -- [2k+2][2k+2]
  let beta0 = matvecmul_row_filt flgsh Xh yh
  -- [2k+2]
  let beta = f32linalg.matvecmul_row Xinv beta0
  -- [2k+2]
  ...
  let y_pred = f32linalg.matvecmul_row (transpose X) beta
  -- [N]

  -- line 4: error
  let y_error = map3 (\flg ye yep -> if flg then (ye - yep) else 0) flgs y y_pred
  -- [N]
