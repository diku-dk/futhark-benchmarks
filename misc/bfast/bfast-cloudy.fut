  ...
  loop A for i < n do
    ...
    let A' =
      map (\ind ->
             ...
             in if v1 == 0.0
                then #[unsafe] A[k * m + j]
                else let x = #[unsafe] (A[j] / v1)
                     in if k < n - 1
                        then -- Ap case
                             #[unsafe] (A[(k + 1) * m + j] - A[(k + 1) * m + i] * x)
                        else x)
          -- irow case
          (iota (n * m))
...
def mat_inv [n] (A: [n][n]f32) : [n][n]f32 =
  ...
  let Ap =
    map (\ind ->
           let (i, j) = (ind / i32.i64 m, ind % i32.i64 m)
           in if j < i32.i64 n
              then #[unsafe] (A[i, j])
...
entry main [m] [N]
  ...
  ---------------------------------------------
  let beta0 =
    map (matvecmul_row_filt Xh) Yh
    |> -- [2k+2]
       opaque
  let beta =
    map2 matvecmul_row Xinv beta0
    |> -- [2k+2]
       opaque
  -- ^ requires transposition of Xinv
  --   unless all parallelism is exploited

  let y_preds =
    map (matvecmul_row Xt) beta
    |> -- [N]
       opaque
  -- ^ requires transposition of Xt (small)
  ...
  let (hs, nss, sigmas) =
    ...
    <| unzip3
       <| map2 (\yh y_error : (i32, i32, f32) ->
                  let ns =
                    map (\ye -> if !(isnan32new ye) then 1i32 else 0) yh
                    |> reduce (+) 0i32
                  let sigma =
                    map (\i -> if i < i64.i32 ns then #[unsafe] y_error[i] else 0.0) (iota n)
                    |> map (\a -> a * a)
                    |> reduce (+) 0.0
  ...
  ---------------------------------------------
  let (_MOs, _MOs_NN, breaks, means) =
    zip (zip4 Nss nss sigmas hs) (zip3 MO_fsts y_errors val_indss)
    |> map (\((Ns: i32, ns: i32, sigma: f32, h: i32), (MO_fst, y_error, val_inds)) ->
              ...
              let (is_break, fst_break) =
                map3 (\mo' b (j: i32) ->
                        if j < Ns - ns && !(isnan32new mo')
                        then -- Validation hack 1: increased break-detection threshold
                             ((f32.abs mo') > 1.0001f32 * b, j)
                        else -- then ( (f32.abs ((f32.abs mo') - b)) > 0.0001, j)
                             (false, j))
                     MO'
                     BOUND
                     (map i32.i64 (iota m))
                |> reduce (\(b1, i1) (b2, i2) ->
                          ...
                          (false, -1i32)
              let mean =
                map2 (\x j -> if j < Ns - ns then x else 0.0) MO' (map i32.i64 (iota m))
                |> reduce (+) 0.0
              let fst_break' =
                if !is_break
                then -1i32
