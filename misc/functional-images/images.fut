...
def mandelbrotEscapes (n: i32) : img (complex, i32) =
  \p1 ->
    let continue (p, i) = i < n && fairlyClose p
...
def visualise_argb_image (img: argb_image)
  ...
  let sizey = ymax - ymin
  let p (y, x) =
    ( xmin + (f32.i64 x * sizex) / f32.i64 screen_width
...
def escape_to_colour (limit: i32)
                     (points: i32)
...
module lys : lys with text_content = text_content = {
  ...
  type state =
    { screen_size: {height: i64, width: i64}
  ...
  def keydown k (s: state) =
    if k == '1'
    then s with mode = #mandelbrot_greyscale
    else if k == '2'
    then s with mode = #julia_greyscale
    else if k == '3'
    then s with mode = #mandelbrot_colour
    else if k == '4'
    then s with mode = #julia_colour
    else if k == '5'
    then s with mode = #figure_7_15
    else if k == '6'
    then s with mode = #fancy
    else if k == SDLK_KP_PLUS
    then s with zooming = -0.01
