...
def iterationSorted [q] [n] [d] [k] [N] [num_leaves] [ppl]
                    ...
                    (query_inds: [n]i32)
                    (i: i32)
                    (visited: *[]i32) : ([][d]f32, [][k](i32, f32), []i32, []i32, []f32, *[N][k](i32, f32), []i32, i32, *[]i32) =
  -- start at old leaf and find a new leaf, until done!
  let (knns', new_leaves, new_stacks, new_dists) =
    unzip4
    <| map5 (\query knn leaf_ind stack dist ->
               let knn' =
                 bruteForce query knn ( leaf_ind * i32.i64 ppl
                                      , leaves[leaf_ind]
                                      )
               let wnn = knn'[k - 1].1
  ...
  let num_valid =
    map (\l -> if l < i32.i64 num_leaves then 1 else 0) new_leaves_all
    |> reduce_comm (+) 0i32
    |> i64.i32
  ...
  let (valid, terminated) =
    ( take num_valid (zip query_inds_all knns_all)
  ...
  let (new_stacks', new_dists') =
    unzip
    <| (gather1D (zip new_stacks new_dists) sort_inds)[:num_valid]
  let visited =
    if (i != 0) && ((i % STEP) == 0)
    then let visited[i / STEP - 1] = i32.i64 num_valid in visited
    else visited
