...
def bruteForce [m] [d] [k]
               (query: [d]f32)
               (knns0: [k](i32, f32))
               (beg: i32, refs: [m][d]f32) : [k](i32, f32) =
  --if query[0] == f32.lowest then copy knns else
  loop (knns) = (copy knns0)
  for i < --for (i,refpt) in (zip (iota m) refs) do
  --  let dist = f32.sqrt <| sumSqrsSeq query refpt in
  i32.i64 m do
    let dist = f32.sqrt <| sumSqrsSeq query (refs[i])
    in if dist > knns[k - 1].1
       then knns
       else -- early exit
            let ref_ind = i + beg
            let (_, _, knns') =
