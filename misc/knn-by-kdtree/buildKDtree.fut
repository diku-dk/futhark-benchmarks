...
def computeTreeShape (m: i32) (defppl: i32) : (i32, i32, i32, i32) =
  ...
  let hp1 = closestLog2 def_num_leaves
  in if hp1 <= 0
     then (-1, 0, m, m)
     else let h = hp1 - 1
...
local
def updateBounds [n] [d2]
                 (level: i32)
                 (median_dims: [n]i32)
                 (median_vals: [n]f32)
                 (node_ind: i32)
                 (lubs_cur: *[d2]f32) : *[d2]f32 =
  let d = d2 / 2
  let ancestor = 0
  let (_, res) =
...
local
def findClosestMed [n] (cur_dim: i32) (median_dims: [n]i32) (node_ind: i32) : i32 =
...
def mkKDtree [m] [d]
             ...
             (q: i64)
             (m': i64)
             (input: [m][d]f32) : (*[m'][d]f32, *[m']i32, *[q]i32, *[q]f32, *[q]i32) =
